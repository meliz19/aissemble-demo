...
def projectDirectory = 'aissemble-demo'
def jenkinsSteps
// Set URL of ArgoCD server
def argocdUrl = ''
def argocdAppName = 'aissemble-demo'
def argocdNamespace = 'argocd'
def argocdDestinationServer = 'https://kubernetes.default.svc'
def argocdBranch = '${branch}'
def argocdSyncLabel = 'argocd.argoproj.io/instance=aissemble-demo'
            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "aiSSEMBLE Demo authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "aiSSEMBLE Demo failed to authenticate properly"
                throw err
            }
        }
    }
...
stage("Deploy") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "aiSSEMBLE Demo deploying"
                ...
                slackSend color: "good",
                        message: "aiSSEMBLE Demo deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "aiSSEMBLE Demo failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "aiSSEMBLE Demo shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "aiSSEMBLE Demo failed to shut down properly"
            throw err
        }
    }
}
